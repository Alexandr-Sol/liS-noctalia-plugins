# Репозиторий плагинов Noctalia

[English](README.md) | [Русский](README.ru.md)

---

Чтобы использовать плагины из этого репозитория, перейдите в настройки Noctalia, во вкладку «Плагины», и добавьте ссылку на репозиторий:

```
https://github.com/Alexandr-Sol/liS-noctalia-plugins
```

---

## Доступные плагины

1. **[AmneziaWG-dkms](amneziawg-dkms)** — Подключение к VPN AmneziaWG через утилиту `awg-quick` с использованием модуля ядра DKMS.
2. **[AmneziaWG-go](amneziawg-go)** — Подключение к VPN AmneziaWG в пространстве пользователя через `amneziawg-go`.
